...
<h3 className='text-2xl'>{weather?.location.country}</h3>
 ...
 <h4 className='mt-3 py-5 text-2xl
 font-bold'>{weather?.current.last_updated}</h4>
 {
 weather?.location && (
 ...
 <img src={"https:" +
 weather?.current.condition.icon ?? ''} alt="icon current condition"
 width={50} height={50} />
 )
 }
 <h2 className='text-3xl
 font-bold'>{weather?.current.temp_c}</h2>
 <p>{weather?.current.condition.text}</p>
 </div>
 <div className="flex gap-10 sm:gap-16 overflow-x-auto
